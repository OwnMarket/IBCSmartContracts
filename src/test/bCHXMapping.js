const helpers = require('./helpers.js')

const BCHXMapping = artifacts.require('./bCHXMapping.sol')

contract('bCHXMapping', accounts => {
    const admin = accounts[0]
    const bscAddress1 = accounts[1]
    const bscAddress2 = accounts[2]

    const chxAddress1 = "CH11111111111111111111111111111111"
    const chxAddress2 = "CH22222222222222222222222222222222"
    const chxAddress3 = "CH33333333333333333333333333333333"

    const signature1 = "signature1"
    const signature2 = "signature2"
    const signature3 = "signature3"

    let bCHXMapping

    beforeEach(async () => {
        bCHXMapping = await BCHXMapping.new()
    })

    it('initializes correctly', async () => {
        assert.equal(await bCHXMapping.owner(), admin, 'Owner address mismatch')
    })

    it('sets mapped address', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)
        
        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        // ACT
        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.notEqual(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 expected to change')
        assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
        assert.notEqual(signature1After, signature1Before, 'Mapped signature 1 expected to change')

        assert.equal(chxAddress1After, chxAddress1, 'Mapped chx address 1 mismatch')
        assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
        assert.equal(signature1After, signature1, 'Mapped signature 1 mismatch')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

    it('sets mapped address for all senders', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        // ACT
        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})
        await bCHXMapping.mapAddress(chxAddress2, signature2, {from: bscAddress2})

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.notEqual(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 expected to change')
        assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
        assert.notEqual(signature1After, signature1Before, 'Mapped signature 1 expected to change')

        assert.equal(chxAddress1After, chxAddress1, 'Mapped chx address 1 mismatch')
        assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
        assert.equal(signature1After, signature1, 'Mapped signature 1 mismatch')

        assert.notEqual(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 expected to change')
        assert.notEqual(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 expected to change')
        assert.notEqual(signature2After, signature2Before, 'Mapped signature 2 expected to change')

        assert.equal(chxAddress2After, chxAddress2, 'Mapped chx address 2 mismatch')
        assert.equal(bscAddress2After, bscAddress2, 'Mapped bsc address 2 mismatch')
        assert.equal(signature2After, signature2, 'Mapped signature 2 mismatch')
    })

    it('rejects address mapping if already exists', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

        // ACT
        await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress3, signature3, {from: bscAddress1}))

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.notEqual(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 expected to change')
        assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
        assert.notEqual(signature1After, signature1Before, 'Mapped signature expected to change')

        assert.equal(chxAddress1After, chxAddress1, 'Mapped chx address 1 mismatch')
        assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
        assert.equal(signature1After, signature1, 'Mapped signature 1 mismatch')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

    it('removes address mapping if called by contract owner', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

        // ACT
        await bCHXMapping.removeMappedAddress(bscAddress1, {from: admin})

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
        assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
        assert.equal(signature1After, signature1Before, 'Mapped signature 1 not expected to change')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

    it('rejects address mapping removal if not called by contract owner', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

        // ACT
        await helpers.shouldFail(bCHXMapping.removeMappedAddress(bscAddress1, {from: bscAddress1}))
        await helpers.shouldFail(bCHXMapping.removeMappedAddress(bscAddress1, {from: bscAddress2}))

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.notEqual(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 expected to change')
        assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
        assert.notEqual(signature1After, signature1Before, 'Mapped signature 1 expected to change')

        assert.equal(chxAddress1After, chxAddress1, 'Mapped chx address 1 mismatch')
        assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
        assert.equal(signature1After, signature1, 'Mapped signature 1 mismatch')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

    it('rejects address mapping removal by contract owner if address mapping does not exist', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        // ACT
        await helpers.shouldFail(bCHXMapping.removeMappedAddress(bscAddress1, {from: admin}))

        // ASSERT
        const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

        const signature1After = await bCHXMapping.signature(chxAddress1)
        const signature2After = await bCHXMapping.signature(chxAddress2)

        assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
        assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
        assert.equal(signature1After, signature1Before, 'Mapped signature 1 not expected to change')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

    it('accepts repeated address mapping if previously removed by contract owner', async () => {
        // ARRANGE
        const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
        const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

        const signature1Before = await bCHXMapping.signature(chxAddress1)
        const signature2Before = await bCHXMapping.signature(chxAddress2)

        await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

        // ACT
        await bCHXMapping.removeMappedAddress(bscAddress1, {from: admin})
        await bCHXMapping.mapAddress(chxAddress3, signature3, {from: bscAddress1})

        // ASSERT
        const chxAddress3After = await bCHXMapping.chxAddress(bscAddress1)
        const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

        const bscAddress1After = await bCHXMapping.bscAddress(chxAddress3)
        const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)
        const bscAddress3After = await bCHXMapping.bscAddress(chxAddress1)

        const signature3After = await bCHXMapping.signature(chxAddress3)
        const signature2After = await bCHXMapping.signature(chxAddress2)
        const signature1After = await bCHXMapping.signature(chxAddress1)

        assert.notEqual(chxAddress3After, chxAddress1Before, 'Mapped chx address 1 expected to change')
        assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
        assert.notEqual(signature3After, signature1Before, 'Mapped signature 1 expected to change')

        assert.equal(chxAddress3After, chxAddress3, 'Mapped chx address 1 mismatch')
        assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
        assert.equal(signature3After, signature3, 'Mapped signature 1 mismatch')

        assert.equal(signature1After, signature1Before, 'Signature from chx address 1 should not exist')
        assert.equal(bscAddress3After, bscAddress1Before, 'Bsc address from chx address 1 should not exist')

        assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
        assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
        assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
    })

/*it('removes address mapping if called by address owner', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

    // ACT
    await bCHXMapping.removeMappedAddress({from: bscAddress1})

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature 1 not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

it('rejects address mapping removal by address owner if address mapping does not exist', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    // ACT
    await helpers.shouldFail(bCHXMapping.removeMappedAddress({from: bscAddress1}))

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature 1 not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

it('accepts repeated address mapping if previously removed by address owner', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    await bCHXMapping.mapAddress(chxAddress1, signature1, {from: bscAddress1})

    // ACT
    await bCHXMapping.removeMappedAddress({from: bscAddress1})
    await bCHXMapping.mapAddress(chxAddress3, signature3, {from: bscAddress1})

    // ASSERT
    const chxAddress3After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress3)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)
    const bscAddress3After = await bCHXMapping.bscAddress(chxAddress1)

    const signature3After = await bCHXMapping.signature(chxAddress3)
    const signature2After = await bCHXMapping.signature(chxAddress2)
    const signature1After = await bCHXMapping.signature(chxAddress1)

    assert.notEqual(chxAddress3After, chxAddress1Before, 'Mapped chx address 1 expected to change')
    assert.notEqual(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 expected to change')
    assert.notEqual(signature3After, signature1Before, 'Mapped signature 1 expected to change')

    assert.equal(chxAddress3After, chxAddress3, 'Mapped chx address 1 mismatch')
    assert.equal(bscAddress1After, bscAddress1, 'Mapped bsc address 1 mismatch')
    assert.equal(signature3After, signature3, 'Mapped signature 1 mismatch')

    assert.equal(signature1After, signature1Before, 'Signature from chx address 1 should not exist')
    assert.equal(bscAddress3After, bscAddress1Before, 'Bsc address from chx address 1 should not exist')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})*/

it('rejects address mapping if chxAddress does not start with "CH"', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    // ACT
    await helpers.shouldFail(bCHXMapping.mapAddress("CF11111111111111111111111111111111", signature1, {from: bscAddress1}))
    await helpers.shouldFail(bCHXMapping.mapAddress("FH11111111111111111111111111111111", signature1, {from: bscAddress2}))

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

it('rejects address mapping if chxAddress ends with character not [A-Z][a-z]0-9]', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    // ACT
    await helpers.shouldFail(bCHXMapping.mapAddress("CH1111111111111111111111111111111 ", signature1, {from: bscAddress1}))
    await helpers.shouldFail(bCHXMapping.mapAddress("CH1111111111111111111111111111111.", signature1, {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress("CH1111111111111111111111111111111/", signature1, {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress("CH1111111111111111111111111111111\\", signature1, {from: bscAddress2}))

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

it('rejects address mapping if signature ends with character not [A-Z][a-z]0-9]', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    // ACT
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress1, "signature1 ", {from: bscAddress1}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, "signature1.", {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, "signature1/", {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, "signature1\\", {from: bscAddress2}))

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

it('rejects address mapping if signature starts with character not [A-Z][a-z]0-9]', async () => {
    // ARRANGE
    const chxAddress1Before = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2Before = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1Before = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2Before = await bCHXMapping.bscAddress(chxAddress2)

    const signature1Before = await bCHXMapping.signature(chxAddress1)
    const signature2Before = await bCHXMapping.signature(chxAddress2)

    // ACT
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress1, " signature1", {from: bscAddress1}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, ".signature1", {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, "/signature1", {from: bscAddress2}))
    await helpers.shouldFail(bCHXMapping.mapAddress(chxAddress2, "\\signature1", {from: bscAddress2}))

    // ASSERT
    const chxAddress1After = await bCHXMapping.chxAddress(bscAddress1)
    const chxAddress2After = await bCHXMapping.chxAddress(bscAddress2)

    const bscAddress1After = await bCHXMapping.bscAddress(chxAddress1)
    const bscAddress2After = await bCHXMapping.bscAddress(chxAddress2)

    const signature1After = await bCHXMapping.signature(chxAddress1)
    const signature2After = await bCHXMapping.signature(chxAddress2)

    assert.equal(chxAddress1After, chxAddress1Before, 'Mapped chx address 1 not expected to change')
    assert.equal(bscAddress1After, bscAddress1Before, 'Mapped bsc address 1 not expected to change')
    assert.equal(signature1After, signature1Before, 'Mapped signature not expected to change')

    assert.equal(chxAddress2After, chxAddress2Before, 'Mapped chx address 2 not expected to change')
    assert.equal(bscAddress2After, bscAddress2Before, 'Mapped bsc address 2 not expected to change')
    assert.equal(signature2After, signature2Before, 'Mapped signature 2 not expected to change')
})

})
